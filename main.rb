...
if player2[:symbol].length > 1 || (player2[:symbol] == player1[:symbol])
  puts 'please Symbol is only one char or choose a different one'
end
...
board_size = gets.chomp until check_class(board_size, 10, 2)

puts 'Choose between 3-5 streaks to win'
win_streak = gets.chomp
win_streak = gets.chomp until check_class(win_streak, 5, 2)
...
until board_size.to_i >= win_streak.to_i
  ...
  board_size = gets.chomp until check_class(board_size, 10, 4)
...
second_player = player2[:starts] ? player1 : player2
...
until player1[:win] || player2[:win]
  ...
  players_move = gets.chomp.to_i until game.valid_move?(players_move, first_player[:symbol], second_player[:symbol])
